    ...
    <div class="col-md-8">
        ...
        <div class="stretch-card">
            <div class="card">
                ...
                <select style="margin-top: 0.5rem; margin-left: 1rem" name="month" onchange="this.form.submit()">
                    <option style="display: none" value="">Chọn Tháng</option>
                    @for ($i = 1; $i <= 12; $i++) <option value="{{ $i }}"
                        {{ request('month') == $i ? 'selected' : '' }}>
                        Tháng {{ $i }}</option>
        ...
        <h4>Bảng thống kê doanh thu theo tháng qua từng năm</h4>
        <table class="table" style="background:white; border:1px solid #dfdfdf">
            <thead>
                <tr>
                    <th>Năm/tháng</th>
                    @for ($month = 1; $month <= 12; $month++) <th style="width:50px">{{ $month }}</th>
                        @endfor
                </tr>
            </thead>
            <tbody>
                @foreach ($revenuetable as $year => $months)
                <tr>
                    <td>{{ $year }}</td>
                    @foreach ($months as $month)
                    <td>{{ number_format($month) }}đ</td>
                    @endforeach
                </tr>
                @endforeach
            </tbody>
